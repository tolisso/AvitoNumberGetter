...
public class ExcelFiller {
    ...
    public static void main (String[] args) throws IOException {
        ExcelFiller ef = new ExcelFiller("Рынок Квартиры 2020", "xlsx", "www.avito", 0, 0, 39,1, 46, 47, 1, 14476);
        ef.fill();
    }
    ...
    public void fill() throws IOException {
        ...
        LocalDateTime date;
        int changedCellsNumber = 0;
        try {
            boolean changed = false;
            for (int i = from; i < to; i++) {
                System.out.println(i);
                var row = sheet.getRow(i);
                if (row.getCell(siteCol).getStringCellValue().equals(site) && (row.getCell(resultCol) == null || row.getCell(resultCol).getStringCellValue().equals("ERROR"))) {
                    changedCellsNumber++;
                    changed = true;
                ...
                if (changed && (changedCellsNumber % 20 == 0 || i == to - 1)) {
                    changed = false;
